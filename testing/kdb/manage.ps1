...
$benchmarkActions = @("benchmark", "benchmark-node")
...
$image = "xqdb-kdb-test:local"
$container = "xqdb-kdb-test"
...
function Build-KdbImage {
    ...
    try {
        $candidateDirectory = Join-Path ([IO.Path]::GetTempPath()) (
            "xqdb-kdb-" + [Guid]::NewGuid().ToString("N")
        )
...
function Stop-KdbContainer {
    try {
        ...
        Invoke-Podman secret rm --ignore xqdb-kdb-license
    }
}
...
function Start-KdbContainer {
    ...
    Stop-KdbContainer
    Invoke-Podman secret create --replace --env=true xqdb-kdb-license KDB_LICENSE_B64
    try {
        if ($HostNetwork) {
...
function Initialize-PythonEnvironment {
    ...
    [Environment]::SetEnvironmentVariable("XQDB_TEST_Q_EXTERNAL", "1", "Process")
    [Environment]::SetEnvironmentVariable("XQDB_TEST_Q_HOST", "127.0.0.1", "Process")
    [Environment]::SetEnvironmentVariable("XQDB_TEST_Q_PORT", "1801", "Process")
    [Environment]::SetEnvironmentVariable("XQDB_Q_ROWS", "$Rows", "Process")
    ...
    [Environment]::SetEnvironmentVariable("SETUPTOOLS_RUST_CARGO_PROFILE", "release", "Process")
    & uv pip install --python $python --reinstall --editable $repoRoot
    if ($LASTEXITCODE -ne 0) {
        throw "setuptools-rust editable install failed with exit code $LASTEXITCODE"
    }
}
